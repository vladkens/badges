...
pub async fn handler(
  ...
  match kind {
    Kind::Stars | Kind::Pulls | Kind::Automated => {
      ...
      match kind {
        Kind::Stars => Ok(badge.for_count("docker stars", rs.stars)),
        Kind::Pulls => Ok(badge.for_count("docker pulls", rs.pulls)),
        Kind::Automated => {
          // todo: colors
          ...
          let color = if rs.automated { Color::Green } else { Color::Yellow };
          Ok(badge.label("docker build").value(value).value_color(color))
        }
        _ => unreachable!(),
      }
    }
    Kind::Version | Kind::Size => {
      ...
      match kind {
        Kind::Version => Ok(badge.for_version("image version", &rs.image_tag)),
        Kind::Size => Ok(badge.for_size("image size", rs.image_size)),
        _ => unreachable!(),
      }
    }
  }
